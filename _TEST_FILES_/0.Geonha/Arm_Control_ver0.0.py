import servo_WR_control_Ver2 as SERVO
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            X_axis = Thread(name="X_axis", target=STEP._STEP_CONTROL_, args=("X", steps, dir, STEPPIN_X,
                                                                             DIRPIN_X, ENPIN_X, STEP_PULSE_LEVEL_TIME))
            Y_axis = Thread(name="Y_axis", target=STEP._STEP_CONTROL_, args=("Y", steps, dir, STEPPIN_Y,
                                                                             DIRPIN_Y, ENPIN_Y, STEP_PULSE_LEVEL_TIME))
            Z_axis = Thread(name="Z_axis", target=STEP._STEP_CONTROL_, args=("Z", steps, dir, STEPPIN_Z,
                                                                             DIRPIN_Z, ENPIN_Z, STEP_PULSE_LEVEL_TIME))
            W_axis = Thread(name="W_axis", target=SERVO._SERVO_CONTROL_, args=(
                PCA, 0, cur_pwms, interval, degree_pwm))
            R_axis = Thread(name="R_axis", target=SERVO._SERVO_CONTROL_, args=(
