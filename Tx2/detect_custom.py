...
def warm_up(device,imgsz,stride,model,half,source):
    t0 = time.time()
    # Padded resize
    img = letterbox(source, imgsz, stride=stride)[0]

    # Convert
    img = source[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
    ...
    print(f"warm up time: {t3-t0} ",end="\n\n")
    return True
...
if __name__=="detect_custom":
    ...
    opt.save_conf=True
    opt.name='elements'
    source = cv2.imread(os.environ["WORK_HOME"]+"/inference/images/bus.jpg",cv2.IMREAD_COLOR)
    # print(source)
    print(opt)
    with torch.no_grad():
       device,imgsz,stride,model,half,save_txt,save_img,view_img=load_model(save_img=True)
       warm_up(device,imgsz,stride,model,half,source)
