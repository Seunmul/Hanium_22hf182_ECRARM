...
# 전역변수 선언
global FRAME, capture, receivedData, detectedData
...
def _detect_(client):
    ...
    global receivedData,detectedData
    if (receivedData["status"] == "starting" or receivedData["status"] == "controlling_finished"):
        ...
        try :
            ...
            send_detector_data(client, status="detecting_finished", classType="ERROR",
                                   x="ERROR", y="ERROR")
        else :
...
if (__name__ == "__main__"):
    try:
        ...
        #디텍터 클라이언트 쓰레드 생성. => 추후에 멀티프로세싱 모듈로 변경 필요.
