...
def _control_(client, Arm):
    ...
    if (receivedData["status"] == "detecting_finished"):
        ...
        x = float(receivedData['Detector']['data']['x'])
        y = float(receivedData['Detector']['data']['x'])
        print(x,y)
        R, x_d = CALCUL.changeCoordinate(x,y)
        ...
        while(float(CALCUL.CHECK_DIS) < vl53_dis.range*0.1) :
            print('>> lidar sensing')
            time.sleep(0.1)
        ...
        _arm_control_(client,Arm, x_d=x_d, y_d=y_d, z_d=z_d, w_d=w_d, r_d=0, s_d=0)
